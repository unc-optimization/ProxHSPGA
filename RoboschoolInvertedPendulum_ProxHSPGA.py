"""@package RoboschoolInvertedPendulum_ProxHSPGA

This package implements the ProxHSPGA algorithm for RoboschoolInvertedPendulum-v1 environment.
...
env = normalize(GymEnv("RoboschoolInvertedPendulum-v1"))

# initialize a neural network policy with a single hidden layer of 8 hidden units
policy = GaussianMLPPolicy(env.spec, hidden_sizes=(16,),learn_std=learn_std)
prev_policy = GaussianMLPPolicy(env.spec, hidden_sizes=(16,),learn_std=learn_std)
...
eval_grad3 = TT.col('eval_grad2',dtype=grad[2].dtype)
eval_grad4 = TT.TensorType(dtype=grad[3].dtype, broadcastable=params[3].broadcastable)('eval_grad3')
if learn_std:
    eval_grad5 = TT.TensorType(dtype=grad[4].dtype, broadcastable=params[4].broadcastable)('eval_grad4')
...
# log directory
log_dir = "log_file/InvertPendulum/ProxHSPGA" + '_lbd_' + str(lbd) +"_lr" + str(learning_rate)
...
for k in range(10):
    ...
    while num_traj<=max_num_traj:
        ...
        # perform update
        params_cur = policy.get_param_values(trainable=True)

        if learn_std:
            f_update(v_est[0],v_est[1],v_est[2],v_est[3],v_est[4])
        else:
            f_update(v_est[0],v_est[1],v_est[2],v_est[3])

        params_hat = policy.get_param_values(trainable=True)
        params_hat = prox_l2_square(params_hat, learning_rate*lbd)
        params_cur = (1 -alpha)*params_cur + alpha*params_hat
        policy.set_param_values(params_cur, trainable=True)
