"""@package MtnCar_HSPGA

This package implements the HSPGA algorithm for MountainCarContinuous-v0 environment.
...
policy = GaussianMLPPolicy(env.spec, hidden_sizes=(8,), learn_std=learn_std)
prev_policy = GaussianMLPPolicy(env.spec, hidden_sizes=(8,), learn_std=learn_std)
...
if learn_std:
    eval_grad5 = TT.TensorType(dtype=grad[4].dtype, broadcastable=params[3].broadcastable)('eval_grad4')
...
# log directory
log_dir = "log_file/MountainCar/HSPGA"+"_lr" + str(learning_rate)
...
for k in range(10):
    ...
    # load policy
    if learn_std:
        file_name = 'mountain_policy' + '.txt'
    else:
        file_name = 'mountain_policy_novar' + '.txt'
